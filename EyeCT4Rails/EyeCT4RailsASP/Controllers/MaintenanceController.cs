        ...
        public ActionResult AddOne(string jobSize, string tramId, string date)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (user != null)
                    {
                        bool succes = MaintenanceRepository.Instance.ScheduleJob((JobSize)Enum.Parse(typeof(JobSize), jobSize), user, Convert.ToInt32(tramId), Convert.ToDateTime(date));
        ...
        public ActionResult AddMore(string jobSize, string tramId, string date, string endDate, string interval)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (user != null)
                    { 
                        bool succes = MaintenanceRepository.Instance.ScheduleRecurringJob((JobSize)Enum.Parse(typeof(JobSize), jobSize), user, Convert.ToInt32(tramId), Convert.ToDateTime(date), Convert.ToInt32(interval), Convert.ToDateTime(endDate));
